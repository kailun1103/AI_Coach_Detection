...
def find_range(input_file):
    ...
    for i, frame_data in enumerate(data):
        if target_part in frame_data and isinstance(frame_data[target_part], dict):
            ...
            # 檢查 y 值
            if wrist_data['y'] is not None and wrist_data['y'] < min_y_value:
                min_y_value = wrist_data['y']
                min_y_frame = i
    
    # 檢查是否找到有效值
    if min_x_frame == -1 or min_y_frame == -1:
        print(f"錯誤：找不到右手腕的有效 {'x' if min_x_frame == -1 else 'y'} 值")
        return -1, -1
    
    # 定義範圍
    start_frame = min_x_frame  # 具有最小 x 的幀
    end_frame = min_y_frame    # 具有最小 y 的幀
...
if __name__ == "__main__":
    input_file = "testing_0224__1_side(2D_trajectory_smoothed).json"
